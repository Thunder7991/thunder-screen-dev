...
export default function() {
  ...
  const headerCenterData = ref({});
  const salesListData = ref([])
  const realRTimeOrderData = ref({ date: [], data: [] });
  ...
  const getKeyValues = ({ key, index }) => {
    const data = headerCenterData.value;
    if (data && data[key]) {
  ...
  const update = () => {
    getUserData().then(data => {
      ...
      headerCenterData.value = _headerData;

      //实时订单数据
      realRTimeOrderData.value = {
        date: [...realRTimeOrderData.value.date, getNowTime()],
        data: [...realRTimeOrderData.value.data, data.realTimeOrder]
      };
