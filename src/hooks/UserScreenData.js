...
export default function() {
  ...
  onMounted(() => {
    task = setInterval(() => {
      ...
      const _headerCenterData = { ...headerCenterData.value };
      _headerCenterData.headerData.forEach(item => {
        item.startVal = item.endVal;
        item.endVal = item.startVal + random(12);
      });
      _headerCenterData.project.forEach(item => {
        let newItem = item.value.replace(/[%]/g, "");
        item.value = `${(+newItem + +Math.random()).toFixed(2)}%`;
      });
      headerCenterData.value = _headerCenterData;
    }, 3000);
  });
  onUnmounted(() => {
